...
draw_moving_shapes(void)
{
  ...
  // moving_circle();
  //draw_circle(cir1.cir_x, cir1.cir_y, cir1.r, COLOR_YELLOW);
...
draw_triangle(void)
{
  int row = (screenHeight/2)+15;
  int col = (screenWidth/4)+7;

  for(int offcol = 0; offcol < 20; offcol++)
    {
      for(int offrow = 0; offrow <= offcol+5; offrow++)
	{
	  drawPixel(col+offcol, row-offrow, COLOR_BLACK);
	}
    }
  ...
  for (step = 0; step < height; step++) {
    ...
    fillRectangle(col - (step / 2), row+step, width, 1, COLOR_BLACK);
  } 
  */
}
  ...
  for(int offcol = 0; offcol < 20; offcol++)
    {
      for(int offrow = 0; offrow <= offcol+5; offrow++)
	{
	  drawPixel(col+offcol, row-offrow, COLOR_YELLOW);
	}
    }
}
